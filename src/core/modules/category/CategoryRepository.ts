...
const CATEGORY_TREE_QUERY_DEFAULT_FIELDS = [
  ...CATEGORY_QUERY_DEFAULT_FIELDS,
  `children {${CATEGORY_QUERY_DEFAULT_FIELDS.join()}}`
]
...
export class CategoryRepository {
  ...
  private static async getCategoryTree(
    optionsGetCategory: OptionsGetCategory<CategoryTreeFields>
  ): Promise<Array<CategoryTree>> {
    const { fields, filter } = optionsGetCategory

    const categoryTreeFields: String = fields
      ? fields.join().replace('children', `children {${CATEGORY_QUERY_DEFAULT_FIELDS.join()}}`)
